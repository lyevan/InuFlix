...
const Card = ({ anime, number }) => {
  return (
    <div className="rounded bg-gray-800 w-30 h-44 lg:w-40 lg:h-60 relative overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300">
      ...
      {number && (
        <p className="text-[0.6rem] lg:text-[0.8rem] text-white bg-[rgba(0,0,0,0.8)] p-2 rounded absolute top-2 right-2 z-10 font-poppins">
          <i className="fa fa-tv text-primary"></i> EP {number || "--"}
        </p>
      )}
    </div>
  );
